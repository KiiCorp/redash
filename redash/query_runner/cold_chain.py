...
from redash.varanus import can_query_securely, can_access
...
class ColdChain(BaseQueryRunner):
    safe_builtins = (
    ...
    def __init__(self, configuration):
        super(ColdChain, self).__init__(configuration)
    ...
    def add_result_column(result, column_name, friendly_name, column_type):
        """Helper function to add columns inside a ColdChain script running in Redash in an easier way
    ...
    @staticmethod
    def execute_shared_query(data_source_name_or_id, query, user, parameters):
        ...
        if not can_query_securely(data_source):
            raise Exception("Data source is not secure: %s." % data_source.name)
        if not can_access(user, data_source):
